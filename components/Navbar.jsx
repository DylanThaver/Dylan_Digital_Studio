        ...
        <ul className="hidden md:flex gap-10 items-center">
          {navItems.map((item) => (
            <li key={item.name} className="relative group">
              ...
              <span className="absolute left-0 -bottom-1 h-0.5 w-full bg-white scale-x-0 origin-left transition-transform duration-300 group-hover:scale-x-100" />
            </li>
          ))}
        </ul>
        ...
        <div className="hidden md:block">
          <Link
            ...
            className="px-5 py-2 rounded-full border border-white/20
                       text-sm text-white hover:bg-linear-to-r 
                       hover:from-orange-500 hover:to-yellow-400
                       hover:text-black shadow-md hover:shadow-orange-400/40
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="absolute top-full left-0 w-full bg-black/95 backdrop-blur-md border-t border-white/10 overflow-hidden md:hidden z-40"
            >
                ...
                {navItems.map((item) => (
                  <li key={item.name}>
                    <Link
                      ...
                      className="block text-white/80 hover:text-white text-base font-medium transition-colors duration-300"
                    >
                      {item.name}
                    </Link>
                  </li>
